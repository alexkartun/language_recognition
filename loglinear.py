...
def create_classifier(in_dim, out_dim):
    ...
    """
    root_six = np.sqrt(6)
    eps_W = root_six / (np.sqrt(in_dim + out_dim))
    eps_b = root_six / (np.sqrt(out_dim))
    W = np.random.uniform(-eps_W, eps_W, [in_dim, out_dim])
    b = np.random.uniform(-eps_b, eps_b, out_dim)
    return [W,b]
